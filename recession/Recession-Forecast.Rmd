...
### pca of spliced
pca         <- prcomp(as.data.frame(res.xts, check.names = FALSE), scale = TRUE, center = TRUE)
pca.var     <- pca$sdev^2
pca.var.per <- pca.var/sum(pca.var)
print(sprintf("Number of components explaining %s = %s, total features = %s", VARIANCE.EXPLAIN.THESH, min(which(cumsum(pca.var.per) >= VARIANCE.EXPLAIN.THESH)), length(pca.var.per)))
barplot(round(pca.var/sum(pca.var) * 100, 1), xlab = "principal-component", ylab="pct. variation")
```

Check PCA till yr 1999:


```{r}
### pca of spliced
pca         <- prcomp(as.data.frame(window(res.xts, end = "1999-12-31"), check.names = FALSE), scale = TRUE, center = TRUE)
pca.var     <- pca$sdev^2
pca.var.per <- pca.var/sum(pca.var)
print(sprintf("Number of components explaining %s = %s, total features = %s", VARIANCE.EXPLAIN.THESH, min(which(cumsum(pca.var.per) >= VARIANCE.EXPLAIN.THESH)), length(pca.var.per)))
barplot(round(pca.var/sum(pca.var) * 100, 1), xlab = "principal-component", ylab = "pct. variation")
