...
getFieldPoints <- function(angle)
{
  ...
  getNewCord <- function(x,y)
  {
    ds <- 0.01
    eField <- E(x,y, Q1, Q2)
    theta <- atan(eField$E_y/eField$E_x)
    x_new <- x + ds * cos(theta)
    y_new <- y + ds * sin(theta)
    return(c(x_new, y_new))
  }
  
  CheckEntry <- function(x,y)
  {
    ((x - D)^2 + (y -0)^2 < R*R) || (x >= D + 5*R)
  }
  ...
  getPoints <- function(x0,y0)
  {
    ...
    while(!entered)
    {
      #print(sprintf("getPoints: X=%s, Y=%s\n", x,y))
      pt <- getNewCord(x,y)
...
getFieldPoints <- function(angle)
{
  ...
  getNewCord <- function(x,y)
  {
    ds <- 0.1
    eField <- E(x,y, Q1, Q2)
    theta <- atan(abs(eField$E_y/eField$E_x))
...
lastpoint <- df60[dim(df45)[1],]
fields <- E(lastpoint$x,lastpoint$y, Q1, Q2)
atan(abs(fields$E_y/fields$E_x)) * 180/pi

# Extend the regression lines beyond the domain of the data
ggplot(df45, aes(x=x, y=y, color=TYPE)) + geom_point(shape='....') +
  scale_colour_hue(l=50)   # Use a slightly darker palette than normal


do.call(rbind, apply(df45[,c("x","y")], 1, function(xy){ c(xy[1], xy[2], E(x=xy[1],y=xy[2], Q1, Q2))}))

#           x         y         E_x         E_y
#62  5.162707  1.330088  6.866021    -51.76825 
#63  5.175855  1.230956  4.601721    -60.75577 
#64  5.183408  1.131242  1.698451    -72.12982 
#65  5.185762  1.03127   -2.111424   -86.83935 
#66  5.188192  1.13124   1.381988    -71.98568 
#67  5.190112  1.031259  -2.475341   -86.64812 
#68  5.192967  1.131218  1.067444    -71.84108 

row = 186
field <- E(x=df45[row,"x"],y= df45[row,"y"], Q1, Q2); field
atan(field$E_y/field$E_x) * 180/pi; sin(atan(field$E_y/field$E_x) )
