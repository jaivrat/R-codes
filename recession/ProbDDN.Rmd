...
simulateData <- function(fit.model, n, current.state)
{
...
current.state <- which(x == max(x))
nextDates <- as.Date(sapply( 1:12, function(i) as.character(last(index(dat.xts)) %m+% months(i))))

getSampleDDN <- function()
{
  tmp.sim.logret <- simulateData(fit.model = fit.model, 12, current.state = current.state)
  tmp.ret.xts    <- xts(tmp.sim.logret, nextDates)
  PerformanceAnalytics::maxDrawdown(tmp.ret.xts)
}

SIM = 100000
ddns <- sapply(1:SIM, function(i) getSampleDDN())
mean(ddns)
```
