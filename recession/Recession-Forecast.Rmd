...
fit.glm <- glmnet(x = mdlX, y = as.matrix(train_set$USREC, ncol = 1), family = "binomial", alpha = 0.55, 
       lambda = 0.008799192,
...
resp.test.xts <- xts(pred.prob.train, index(train_set))
resp.test.xts <- merge.xts(resp.test.xts, train_set$USREC)
plot(resp.test.xts)
```
...
resp.xts <- xts(pred.prob, index(test_set.x.scaled))
resp.xts <- merge.xts(resp.xts, test_set$USREC)
